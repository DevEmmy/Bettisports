          ...
          {navItems?.map((item, i) => {
            return (
              <>
                {item?.sub ? (
                  <div
                    className={`relative cursor-pointer p-3 flex justify-between  text-sm ${
                      active == i ? 'bg-primary1 text-white' : 'text-gray-500'
                    }`}
                    key={i}
                    onClick={() => setActive(i)}>
                      ...
                      <div className='absolute top-14 flex flex-col bg-white  shadow-xl w-full text-xs -left-2 z-50'>
                        {subToShow?.map((s: any, i: string) => {
                          return (
                            <Link
                              href={s.link}
                              onClick={() => setSubToShow(null)}
                              key={i}
                              className='hover:bg-gray-200 p-4 text-gray-500'>
                              {s.title}
                            </Link>
                          );
                        })}
                  ...
                  <Link
                    onClick={() => setActive(i)}
                    href={item?.link}
                    className={`flex-center gap-2 cursor-pointer py-4 px-4 ${
                      active == i ? 'bg-primary1 text-white' : 'text-gray-500'
                    }`}
                    key={i}>
                    <p className=' '>{item?.icon}</p>
